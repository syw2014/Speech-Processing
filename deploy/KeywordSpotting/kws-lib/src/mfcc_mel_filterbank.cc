...
bool MfccMelFilterbank::Initialize(int input_length, double input_sample_rate,
  ...
  if (num_channels_ < 1) {
    std::cout << "[ERROR]:Number of filterbank channels must be positive.";
    return false;
  }

  if (sample_rate_ <= 0) {
    std::cout << "[ERROR]:Sample rate must be positive.";
    return false;
  }

  if (input_length < 2) {
    std::cout << "[ERROR]:Input length must greater than 1.";
    return false;
  }

  if (lower_frequency_limit < 0) {
    std::cout << "[ERROR]:Lower frequency limit must be nonnegative.";
    return false;
  }

  if (upper_frequency_limit <= lower_frequency_limit) {
    std::cout << "[ERROR]:Upper frequency limit must be greater than "
  ...
  if (!bad_channels.empty()) {
    std::cout << "[ERROR]:Missing " << bad_channels.size() << " bands "
...
void MfccMelFilterbank::Compute(const std::vector<double> &input,
  ...
  if (!initialized_) {
    std::cout << "[ERROR]:Mel Filterbank not initialized.";
    return;
  }

  if (input.size() <= end_index_) {
    std::cout << "[ERROR]:Input too short to compute filterbank";
    return;
  }
