 ...
 * @LastEditTime: 2021-08-04 16:18:42
 * @LastEditors: Please set LastEditors
 * @Description: In User Settings Edit
 * @FilePath: \deploy\tfversion\mfcc_dct.cc
 */
...
bool MfccDct::Initialize(int input_length, int coefficient_count) {
  ...
  if (coefficient_count_ < 1) {
    std::cout << "[ERROR]:Coefficient count must be positive.";
    return false;
  }

  if (input_length < 1) {
    std::cout << "[ERROR]:Input length must be positive.";
    return false;
  }

  if (coefficient_count_ > input_length_) {
    std::cout << "[ERROR]:Coefficient count must be less than or equal to "
               << "input length.";
    return false;
  }
...
void MfccDct::Compute(const std::vector<double> &input,
  ...
  if (!initialized_) {
    std::cout << "[ERROR]:DCT not initialized.";
    return;
  }
