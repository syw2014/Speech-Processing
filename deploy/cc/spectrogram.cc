...
bool Spectrogram::Initialize(const std::vector<double>& window,
  ...
  if (window_length_ < 2) {
    std::cout << "[ERROR]:Window length too short.";
    initialized_ = false;
    return false;
  }
  ...
  if (step_length_ < 1) {
    std::cout << "[ERROR]:Step length must be positive.";
    initialized_ = false;
    return false;
  }
  ...
  initialized_ = true;
  if (!Reset()) {
    std::cout << "[ERROR]:Failed to Reset()";
    return false;
  }
  return true;
}
...
bool Spectrogram::Reset() {
  if (!initialized_) {
    std::cout << "[ERROR]:Initialize() has to be called, before Reset().";
    return false;
  }
...
bool Spectrogram::ComputeComplexSpectrogram(
  ...
  if (!initialized_) {
    std::cout << "[ERROR]: ComputeComplexSpectrogram() called before successful call "
               << "to Initialize().";
    return false;
  }
  // CHECK(output);
  ...
  while (GetNextWindowOfSamples(input, &input_start)) {
    assert(input_queue_.size() == window_length_);
...
bool Spectrogram::ComputeSquaredMagnitudeSpectrogram(
  ...
  if (!initialized_) {
    std::cout << "[ERROR]: ComputeSquaredMagnitudeSpectrogram() called before "
